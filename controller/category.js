...
class Category {
    ...
    static async get (ctx) {
        ...
        if (currentPage && pageSize) {
            ...
            const categorys = await CategoryModel.paginate(query, options)
    ...
    static async delete (ctx) {
        ...
        // 如果删除项为别的项的super
        let result = await CategoryModel.find({ category: id })

        if (result.length) {
            // 更新子项的super
            await CategoryModel.find({ '_id': { $in: Array.from(result, c => c._id) } }).update({ $set: { category: isExist.category || null } })
        }
